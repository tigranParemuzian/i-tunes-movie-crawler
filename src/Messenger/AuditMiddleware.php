...
class AuditMiddleware implements MiddlewareInterface
{
    ...
    public function handle(Envelope $envelope, StackInterface $stack): Envelope
    {
        ...
        if($envelope->last(ReceivedStamp::class)) {
            $this->messengerAuditLogger->info('[{id}] Receive {class}', $context);
        }elseif($envelope->last(SentStamp::class)){
            $this->messengerAuditLogger->info('[{id}] send {class}', $context);
        }else{
            $this->messengerAuditLogger->info('[{id}] Receive or sending {class}', $context);
        }
